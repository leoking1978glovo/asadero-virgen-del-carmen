    ...
    <div className={`flex flex-col h-full bg-white ${className}`} data-chat-widget>
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 bg-white" aria-live="polite">
        ...
        {!isLoading && !error && messages.length === 0 && !pendingMessage && (
          <p className="text-lg text-[#666666] text-center py-8">¡Hola! Soy {CHAT_AGENT_NAME}, el asistente virtual con ia del Asador Virgen del Carmen 👋 ¿Te ayudo con tu pedido?</p>
        )}
        {messages.map((message, index) => {
          ...
          return (
            <div key={`${message.id}-${index}`} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[95%] text-lg leading-relaxed ${isUser ? 'bg-primary text-cream rounded-2xl rounded-br-sm px-4 py-2.5' : 'bg-transparent text-[#1a1a1a] px-0 py-1'}`} style={!isUser ? { whiteSpace: 'pre-line', wordBreak: 'keep-all' } : undefined}>
                {text}
              </div>
            </div>
          );
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2 px-3 py-3 border-t border-[#e5e5e5] bg-white">
        <input ref={inputRef} type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Escribe tu mensaje..." disabled={isLoading || !!error} className="flex-1 rounded-lg border border-[#d4d4d4] bg-[#f5f5f5] px-4 py-2.5 text-lg text-[#1a1a1a] placeholder:text-[#a3a3a3] focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary disabled:opacity-50" aria-label="Mensaje para el asistente" />
        <button type="submit" disabled={isLoading || isTyping || !!error || !inputValue.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary text-cream transition-colors hover:bg-primary/90 disabled:opacity-40" aria-label="Enviar mensaje">
