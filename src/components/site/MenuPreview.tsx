...
export function MenuPreview() {
  ...
  // Elecciones del cliente DENTRO de la ventanita (combos con opciones).
  ...
  const handleConfirm = () => {
    ...
    if (selectedItem.choices && selectedItem.choices.length > 0) {
      const chosen = selectedItem.choices.map((_, i) => modalChoices[i]);
  ...
  return (
    <section id="menu" className="relative bg-[#F8E9CC] py-16 md:py-24">
      ...
      <div ref={viewportRef} className="mt-8 overflow-hidden md:mt-12">
        <motion.div
          ...
          dragElastic={0.08}
          onDragEnd={onDragEnd}
          className="flex cursor-grab gap-5 px-4 active:cursor-grabbing md:gap-6 md:px-12"
        >
          {menu.map((cat, i) => (
            <motion.div
              key={cat.title}
                        ...
                        {group.options.map((option) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                toggleModalChoice(groupIndex, option)
                              }
